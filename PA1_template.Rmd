...
```{r Load and cache the data, cache = TRUE}
...
- 5-minute interval with the maximum number of steps is at the `r MaxStepInterval` minute. 
...
```{r Calculate the total number of missing values}
missing <- sum(!complete.cases(activity))
```

- The total number of missing values in variable `steps` is `r missing`.

### Create a new dataset that is equal to the original dataset but with the missing data filled in

```{r Impute the missing values with the mean for that 5-minutes interval}
...
```{r Calculating total steps per day, but imputed}
...
```{r Make-a-histogram-again-but-imputed}
...
```{r Arrive with mean and median of total number of steps each day in imputed data set}
